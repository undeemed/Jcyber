...
def loop(slug):                                   # orchestrator process
    ...
    while eng_active(slug) and within_budget(cfg.budget):
        ...
        state   = render_state(snap, priors, notes, budget_left(cfg),
                               cap=cfg.jev.state_budget_chars)
        q       = catalog.assemble(snap, skills)     # decision-catalog.md
        ans     = jev.system_one(state, q, model=cfg.jev.model)
        cls, tool, target = resolve(ans)             # class table §3

        if not in_scope(target, scope):              # gate 1: string match,
            record(ans, "blocked");  return HALT(scope_violation)   # never argued over
        if ans.scope_safe.noul < cfg.gates.scope_model_floor.auto:  # 0.90
            record(ans, "confirm_parked");  park(ans);  continue
        thr = {recon_passive: 0.80, recon_active: 0.85, probing: 0.90,
               fuzzing: 0.95, report: 0.95, exploit: 0.95}[cls]
        if ans.next_action.confidence < thr:
            record(ans, "confirm_parked" if cls != recon_* else "queued")
            park(ans);  continue
        record(ans, "auto")

        rate_gate(target, scope.constraints)         # 5 rps + 08:00-20:00 UTC window
        ...
        if ans.report_ready.noul >= 0.95:            # else: confirm (operator signs)
            render_report(slug)                     # priority = 0.7·sev + 0.3·bounty (code)
            tdb.commit(slug, distill(slug));  return CLOSE
...
## 3. `next_action` class → HexStrike routing

Tool names below are the **exact MCP tool names** verified from
`hexstrike_mcp.py` (HexStrike v6, `master`; 151 named tools). Router picks the
first tool whose param preconditions are met (e.g. creds only for classes
they need; account names only when in `scope.toon`).
...
| probing | `nuclei_scan`, `sqlmap_scan`, `wpscan_analyze`, `dalfox_xss_scan`, `xsser_scan`, `jaeles_vulnerability_scan`, `jwt_analyzer`, `api_fuzzer`, `graphql_scanner`, `comprehensive_api_audit`, `arjun_scan`, `qsreplace`, `http_repeater`, `browser_agent_inspect`, `netexec_scan`, `smbmap_scan`, `enum4linux_scan`, `bugbounty_authentication_bypass_testing` | target: state (E-###/Endpoint ids); template/tag sets + accounts: config / scope.toon `authorized_accounts` |
...
endpoints. The "reflex" role belongs to Jev (PLAN §2); letting two decision
layers argue about the next action is the failure mode this architecture
exists to prevent.
...
template lists, msf modules+options). Jev answers select *which* tool and
*which element of a closed set* — they never supply the string. This is what
"no free-form command construction" means at the call site.
